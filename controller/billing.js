...
exports.createBilling = async (req, res) => {
    try {
        ...
        const allReservationsAgg = await ORDER_BOOKING.aggregate([
            { $match: { product: { $in: allRelevantProductIds }, isDeleted: { $ne: true }, status: "Hold" } },
        ...
        for (const productInfo of productsInvolved) {
            ...
            // Calculate available physical sets for this specific product
            let physicalInInv = physicalCountMap[pId] || 0;
            
            const sizeCounts = (productInfo.sizes || []).map((s) => {
                let sReturn = returnCountMap.get(`${pId}_${s._id.toString()}`) || 0;
                return physicalInInv + sReturn;
            });
            const totalPhysicalSets = sizeCounts.length > 0 ? Math.min(...sizeCounts) : physicalInInv;

            const totalReservedAll = reservedCountMap[pId] || 0;

            let availableForMe;
            let quotaMessage = "";
            if (requiredByReservation > 0) {
                availableForMe = requiredByReservation;
                quotaMessage = `Your selected reservation limit for ${productInfo.productCode} (${productInfo.sizes?.length} sizes) is ${requiredByReservation} sets.`;
            } else {
    ...
    try {
        ...
        if (availableQuota <= alreadyScanned) {
            return res.status(400).json({ 
                success: false, 
                message: isReserved 
                    ? `Order Limit: You already scanned ${alreadyScanned} of the ${selectedReservationTotal} reserved sets for this item.`
