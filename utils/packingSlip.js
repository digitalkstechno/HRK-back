...
const COLS = [
  ...
  { h: "Qty",       w: 125, align: "center" },
];
const TABLE_W = COLS.reduce((s, c) => s + c.w, 0); // 535 = CONTENT_W
...
function generatePackingSlipPDF(billing, res) {
  ...
  if (fs.existsSync(logoPath)) {
    ...
    doc.fontSize(18).font("Helvetica-Bold").fillColor("#000")
       .text("HRK", 0, y + 8, { width: PAGE_W, align: "center", lineBreak: false });
  }
  ...
  phones.forEach((p, i) => {
    doc.text(p, PAGE_W - M - 110, y + i * 12, { width: 110, align: "right", lineBreak: false });
  });

  // PACKING SLIP title — left side
  doc.fontSize(16).font("Helvetica-Bold").fillColor("#000")
     .text("PACKING SLIP", M, y + 8, { width: 160, lineBreak: false });
  ...
  infoRows.forEach(({ l, r }, i) => {
    const ry = y + i * INFO_ROW_H + (INFO_ROW_H - 8.5) / 2;
  ...
  const totalAmt = grouped.reduce((s, r) => s + r.total, 0);

  doc.fontSize(9).font("Helvetica-Bold").fillColor("#000")
     .text(`Total Qty : ${totalQty}`, M, y + (TOTAL_H - 9) / 2, {
  ...
  doc.fontSize(8).font("Helvetica").fillColor("#000")
     ...
     .text("Subject to Surat Jurisdiction.", M + 6, y + 18, { lineBreak: false });

  // Horizontal divider above signatures
  doc.moveTo(M, y + 32).lineTo(M + CONTENT_W, y + 32).lineWidth(0.3).stroke("#aaaaaa");

  // Signature labels
  doc.fontSize(8).font("Helvetica").fillColor("#000")
     .text("Packed By : _____________________", M + 6, y + 38, { lineBreak: false });
  doc.text("Checked By : ____________________", sigDivX + 6, y + 38, { lineBreak: false });
