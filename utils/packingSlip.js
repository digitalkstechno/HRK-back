...
const COLS = [
  ...
  { h: "Color/SKU", w: 150, align: "center" },
  { h: "Price",     w: 90,  align: "center" },
  { h: "Qty",       w: 100, align: "center" },
];
...
function drawTable(doc, startX, startY, cols, rows, HDR_H, ROW_H) {
  ...
  cols.forEach((col, i) => {
    ...
    doc.text(col.h.toUpperCase(), cx + 5, startY + (HDR_H - 9) / 2 + 1, {
  ...
  rows.forEach((cells, idx) => {
    ...
    cells.forEach((val, ci) => {
      ...
      doc.fontSize(9)
         ...
         .fillColor("#000000")
         .text(String(val), cx + 5, ry + (ROW_H - 9) / 2 + 1, {
  ...
  for (let i = 1; i < totalRows; i++) {
    ...
    doc.moveTo(startX, ry).lineTo(startX + totalW, ry).lineWidth(0.3).stroke("#dddddd");
  }
...
function generatePackingSlipPDF(billing, res) {
  ...
  phones.forEach((p, i) => {
    doc.text(p, PAGE_W - M - 120, y + i * 13, { width: 120, align: "right", lineBreak: false });
  });

  doc.fontSize(18).font("Helvetica-Bold").fillColor("#000")
     .text("PACKING SLIP", M, y + 10, { width: 200, lineBreak: false });
  ...
  doc.moveTo(rX, y).lineTo(rX, y + infoBlockH).lineWidth(1).stroke("#000000");

  for (let i = 1; i < infoRows.length; i++) {
    doc.moveTo(M, y + i * INFO_ROW_H)
       .lineTo(M + CONTENT_W, y + i * INFO_ROW_H)
       .lineWidth(0.5).stroke("#bbbbbb");
  }

  doc.fontSize(10);
  infoRows.forEach(({ l, r }, i) => {
    const ry = y + i * INFO_ROW_H + (INFO_ROW_H - 10) / 2;

    doc.font("Helvetica-Bold").fillColor("#000")
       .text(l[0], M + 8, ry, { lineBreak: false, width: 80 });
    const lw = doc.widthOfString(l[0]);
    doc.font("Helvetica").fillColor("#000")
       .text(l[1], M + 8 + lw, ry, { lineBreak: false, width: lW - lw - 15 });

    doc.font("Helvetica-Bold").fillColor("#000")
       .text(r[0], rX + 8, ry, { lineBreak: false, width: 90 });
    const rw = doc.widthOfString(r[0]);
    doc.font("Helvetica").fillColor("#000")
       .text(r[1], rX + 8 + rw, ry, { lineBreak: false, width: rW - rw - 15 });
  });
  ...
  doc.fontSize(10).font("Helvetica-Bold").fillColor("#000")
     .text(`TOTAL QTY : ${totalQty} SETS`, M, y + (TOTAL_H - 10) / 2, {
  ...
  doc.fontSize(8).font("Helvetica").fillColor("#000")
     .text("1. Received the above goods in good condition.", M + 10, footerY + 10, { lineBreak: false })
     .text("2. Subject to Surat Jurisdiction.", M + 10, footerY + 22, { lineBreak: false });
  ...
  // Signature labels
  doc.fontSize(9).font("Helvetica-Bold").fillColor("#000")
     .text("PACKED BY : _____________________", M + 10, footerY + 48, { lineBreak: false });
  doc.text("CHECKED BY : ____________________", sigDivX + 10, footerY + 48, { lineBreak: false });
